    ...
    async def _async_launch_request(self, messages, max_tokens, extra_headers=None):
        ...
        tasks = [
            self.client.chat.completions.create(
                ...
                max_tokens=1,
                stream=True,
                extra_headers=extra_headers,  # Very short response
            ),
            self.client.chat.completions.create(
                model=self.model,
                messages=messages,
                max_tokens=max_tokens,
                stream=True,
                extra_headers=extra_headers,  # Normal response
            ),
        ]

        responses = await asyncio.gather(*tasks)
        short_response, normal_response = responses

        # Process the normal response
        async for chunk in normal_response:
            ...
            if chunk_message is not None:
                ...
                words += chunk_message
                # Count tokens roughly (1 token per 4 characters)

        # print(normal_response)

        # Process the short response

        print("\nNormal response (max_tokens=50):")
        print(words)

        # Estimate prompt tokens (roughly 1 token per 4 characters)
        tokens_prefill = len(self.tokenizer(messages[0]["content"]).input_ids)
        tokens_out = len(self.tokenizer(words).input_ids)
        return Response(
            body=words,
