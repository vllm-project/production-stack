...
from vllm_router.parsers.granian_parser import parse_granian_args
...
def main():
    """
    Main entry point for running the application with Granian 2.0+.
    """
    # Use the Granian-specific parser
    args = parse_granian_args()

    # Create and initialize FastAPI app
    app = create_app()
    initialize_all(app, args)

    # Set ulimit to avoid connection issues
    set_ulimit()
    ...
    try:
        server = Server(
            ...
            port=args.port,
            interface="asgi",
            runtime_threads=args.granian_threads,
            task_impl=args.granian_loop,
            log_level=args.granian_log_level,
        )

        # Start the server using the async serve method
        asyncio.run(server.serve())

    except Exception as e:
        logger.error(f"Error starting Granian server: {e}")
        logger.info("Falling back to uvicorn server")
        import uvicorn
        ...
        uvicorn.run(
            ...
            port=args.port,
            log_level=(
                args.granian_log_level.lower()
                if hasattr(args, "granian_log_level")
                else "info"
            ),
        )
