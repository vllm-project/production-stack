...
class ExtProcService(ExternalProcessorBase):
    ...
    def __init__(self, name: str = "VLLMRouterExtProc"):
        ...
        if not semantic_cache_available:
            logger.warning(
                "Semantic cache is not available. The extproc service will pass through all requests."
            )
        elif not is_semantic_cache_enabled():
            logger.warning(
                "Semantic cache is not enabled. The extproc service will pass through all requests."
            )
    ...
    async def _handle_processing_request(
        self, request_id: str, request: ProcessingRequest
        ...
        if request.HasField("request_headers"):
            return await self._handle_request_headers(
                request_id, request.request_headers
            )
            ...
            return await self._handle_response_headers(
                request_id, request.response_headers
            )
    ...
    async def _handle_request_headers(
        self, request_id: str, headers
    ) -> ProcessingResponse:
        """
        ...
        if body.end_of_stream and context["is_chat_completion"]:
            try:
                ...
                if (
                    ...
                    and is_semantic_cache_enabled()
                    and not context["skip_cache"]
                ):
                    ...
                    if semantic_cache:
                        # Search the cache
                        similarity_threshold = body_json.get(
                        ...
                        semantic_cache_latency.labels(server="router").set(
                            cache_latency
                        )
                        ...
                        if cache_result:
                            ...
                            return self._create_immediate_response_with_body(
                                ...
                                headers=[
                                    ...
                                    (
                                        "x-similarity-score",
                                        str(cache_result["similarity_score"]),
                                    ),
                                ],
                            )
                        else:
    ...
    async def _handle_response_body(self, request_id: str, body) -> ProcessingResponse:
        ...
        if (
            body.end_of_stream
            and context["is_chat_completion"]
            ...
            try:
                ...
                if semantic_cache and not context["skip_cache"]:
                    ...
                    if hasattr(semantic_cache, "db") and hasattr(
                        semantic_cache.db, "index"
                    ):
                ...
                logger.error(
                    f"Error storing in cache for request {request_id}: {str(e)}"
                )
    ...
    def _create_immediate_response(self) -> ProcessingResponse:
        ...
        Returns:
            A ProcessingResponse
        """
        return ProcessingResponse(immediate_response=ImmediateResponse())
    ...
    def _create_body_request_response(self) -> ProcessingResponse:
        ...
        Returns:
            A ProcessingResponse
        """
        return ProcessingResponse(request_body=BodyResponse())

    def _create_immediate_response_with_body(
        ...
        for name, value in headers:
            header_mutations.append(HeaderValue(key=name, value=value))

        return ProcessingResponse(
            immediate_response=ImmediateResponse(
                status=HttpStatus(code=200),
                headers=HeaderMutation(set_headers=header_mutations),
                body=body,
            )
        )
...
async def _serve_extproc_async(service, port: int = 50051, grace_period: int = 5):
    ...
    if not extproc_available:
        logger.error(
            "Envoy extproc protobuf definitions not available. Cannot start extproc service."
        )
        return
        ...
        "envoy.service.ext_proc.v3.ExternalProcessor",
        {
            "Process": grpc.stream_stream_rpc_method_handler(
                ...
                request_deserializer=(
                    ProcessingRequest.FromString
                    if hasattr(ProcessingRequest, "FromString")
                    else None
                ),
                response_serializer=(
                    ProcessingResponse.SerializeToString
                    if hasattr(ProcessingResponse, "SerializeToString")
                    else None
                ),
            )
        },
    ...
    for signal_name in ("SIGINT", "SIGTERM"):
        ...
        try:
            loop.add_signal_handler(
                getattr(signal, signal_name), lambda: asyncio.create_task(_shutdown())
            )
