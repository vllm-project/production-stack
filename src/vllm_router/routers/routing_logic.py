...
class DisaggregatedPrefillRouter(RoutingInterface):
    ...
    Supports conditional disaggregation: requests with estimated input tokens below
    `routing_threshold` are routed directly to a decoder (bypassing DPD), since local
    chunked prefill is faster for short inputs than remote prefill + KV transfer.

    Also supports round-robin load balancing across multiple prefill and decode endpoints.
    """
    ...
    @staticmethod
    def _estimate_input_tokens(request_json: Dict) -> int:
        """Estimate input token count from request body using char/4 heuristic."""
    ...
    def should_disaggregate(self, request_json: Dict) -> bool:
        """Determine if request should go through DPD path based on input length.

        Returns True if the request should be disaggregated (sent to prefiller first),
        False if it should be sent directly to decoder for local prefill.
        A threshold of 0 means always disaggregate (original behavior).
        """
    ...
    def route_request(
        ...
        First request goes to prefill endpoint, then second request goes to decode endpoint.
        Uses round-robin across multiple prefill/decode endpoints.
        """
