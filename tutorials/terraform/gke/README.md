...
The observability stack is based on [kube-prometheus-stack](https://github.com/prometheus-community/helm-charts/blob/main/charts/kube-prometheus-stack/README.md).

After installing, the dashboard can be accessed through the service `service/vllm-grafana`.
...
```bash
kubectl port-forward svc/vllm-grafana 3000:80 --address 0.0.0.0
```
...
Open the webpage at `http://<IP of your node>:3000` to access the Grafana web page. The default user name is `admin` and the password can be configured in `values.yaml` (default is generated by Helm and stored in a secret).
...
We provide a minimal example of how to use the Prometheus Adapter to export vLLM metrics in the [helm chart values](../../../helm/values.yaml).
