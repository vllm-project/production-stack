...
**Note**: The VLLMRuntime configurations in this tutorial reference this secret as `huggingface-token`. If you use a different secret name, make sure to update the `hfTokenSecret.hfTokenSecretName` field in your VLLMRuntime configurations accordingly.
...
**Note**: The sample YAML does not specify a `storageClassName`, so Kubernetes will use the cluster's default StorageClass. If you need a specific StorageClass, add `storageClassName` to the `storageConfig` sections in both prefill and decode. You can check available StorageClasses with `kubectl get sc`.
...
4. **Prefill pods restarting with NIXL/UCX errors**: If you see `cuStreamCreate failed: invalid device context` in pod logs, upgrade to `lmcache/vllm-openai:v0.3.13` or later which includes NIXL >= 0.9.0 with the UCX CUDA context fix.

5. **LMCache ABI errors with latest image**: If you see `ImportError: undefined symbol` errors with `lmcache/vllm-openai:latest`, use a pinned version like `v0.3.13` instead, as the latest tag may have ABI incompatibilities between vLLM and LMCache.
