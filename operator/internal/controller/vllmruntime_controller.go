...
func (r *VLLMRuntimeReconciler) deploymentForVLLMRuntime(
	...
	env := []corev1.EnvVar{}
	// vLLM defaults to V1; only set VLLM_USE_V1=0 when user explicitly disables it
	if vllmRuntime.Spec.VLLMConfig.V1 != nil && !*vllmRuntime.Spec.VLLMConfig.V1 {
	...
	if vllmRuntime.Spec.LMCacheConfig.Enabled {
		...
		// Add KV transfer config based on V1 flag (defaults to true)
...
func (r *VLLMRuntimeReconciler) reconcilePrefillResources(ctx context.Context, vllmRuntime *productionstackv1alpha1.VLLMRuntime) error {
	...
	if err != nil && errors.IsNotFound(err) {
		...
		desired := r.serviceForNode(vllmRuntime, &vllmRuntime.Spec.Topology.Prefill, "prefill")
		if !reflect.DeepEqual(foundService.Spec.Ports, desired.Spec.Ports) ||
			!reflect.DeepEqual(foundService.Spec.Selector, desired.Spec.Selector) {
			foundService.Spec.Ports = desired.Spec.Ports
			foundService.Spec.Selector = desired.Spec.Selector
			log.Info("Updating Service", "Service.Name", foundService.Name)
			if err := r.Update(ctx, foundService); err != nil {
				log.Error(err, "Failed to update Service", "Service.Name", foundService.Name)
				return err
			}
		}
	}
	...
	if vllmRuntime.Spec.Topology.Prefill.StorageConfig.Enabled {
		...
		if err != nil && errors.IsNotFound(err) {
			...
			// Update existing PVC if storage size changed
			desired := r.pvcForNode(vllmRuntime, &vllmRuntime.Spec.Topology.Prefill, "prefill")
			if !reflect.DeepEqual(foundPVC.Spec.Resources, desired.Spec.Resources) {
				foundPVC.Spec.Resources = desired.Spec.Resources
				log.Info("Updating PVC", "PVC.Name", foundPVC.Name)
				if err := r.Update(ctx, foundPVC); err != nil {
					log.Error(err, "Failed to update PVC", "PVC.Name", foundPVC.Name)
					return err
				}
			}
		}
	...
	if err != nil && errors.IsNotFound(err) {
		...
		// Update existing deployment if key fields changed
		desired := r.deploymentForNode(vllmRuntime, &vllmRuntime.Spec.Topology.Prefill, "prefill")
		if nodeDeploymentNeedsUpdate(found, desired) {
			...
			log.Info("Updating Deployment", "Deployment.Name", found.Name)
			if err := r.Update(ctx, found); err != nil {
				log.Error(err, "Failed to update Deployment", "Deployment.Name", found.Name)
				return err
			}
		}
	}
...
func (r *VLLMRuntimeReconciler) reconcileDecodeResources(ctx context.Context, vllmRuntime *productionstackv1alpha1.VLLMRuntime) error {
	...
	if err != nil && errors.IsNotFound(err) {
		...
		// Update existing service if spec changed
		desired := r.serviceForNode(vllmRuntime, &vllmRuntime.Spec.Topology.Decode, "decode")
		if !reflect.DeepEqual(foundService.Spec.Ports, desired.Spec.Ports) ||
			!reflect.DeepEqual(foundService.Spec.Selector, desired.Spec.Selector) {
			foundService.Spec.Ports = desired.Spec.Ports
			foundService.Spec.Selector = desired.Spec.Selector
	...
	if vllmRuntime.Spec.Topology.Decode.StorageConfig.Enabled {
		...
		if err != nil && errors.IsNotFound(err) {
			...
			// Update existing PVC if storage size changed
			desired := r.pvcForNode(vllmRuntime, &vllmRuntime.Spec.Topology.Decode, "decode")
			if !reflect.DeepEqual(foundPVC.Spec.Resources, desired.Spec.Resources) {
				foundPVC.Spec.Resources = desired.Spec.Resources
				log.Info("Updating PVC", "PVC.Name", foundPVC.Name)
				if err := r.Update(ctx, foundPVC); err != nil {
	...
	if err != nil && errors.IsNotFound(err) {
		...
		// Update existing deployment if key fields changed
		desired := r.deploymentForNode(vllmRuntime, &vllmRuntime.Spec.Topology.Decode, "decode")
		if nodeDeploymentNeedsUpdate(found, desired) {
			found.Spec.Replicas = desired.Spec.Replicas
			found.Spec.Template.Spec.Containers = desired.Spec.Template.Spec.Containers
			found.Spec.Template.Spec.InitContainers = desired.Spec.Template.Spec.InitContainers
...
// nodeDeploymentNeedsUpdate checks if a PD node deployment needs updating by comparing key fields.
// We only compare image and args to avoid false positives from Kubernetes-injected default values
// on fields like Env, Resources, Ports, etc.
func nodeDeploymentNeedsUpdate(current, desired *appsv1.Deployment) bool {
	...
	for i := range desiredContainers {
		if currentContainers[i].Image != desiredContainers[i].Image {
			return true
		}
		if !reflect.DeepEqual(currentContainers[i].Args, desiredContainers[i].Args) {
			return true
		}
	}
	return false
...
func (r *VLLMRuntimeReconciler) buildVLLMArgsForNode(nodeConfig *productionstackv1alpha1.NodeConfig, nodeType string) []string {
	...
	if nodeConfig.LMCacheConfig.Enabled {
		...
		var lmcache_config string
		if nodeConfig.VLLMConfig.V1 == nil || *nodeConfig.VLLMConfig.V1 {
